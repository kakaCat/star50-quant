#!/usr/bin/env python3
"""
Spyder runnable LightGBM rolling-window pipeline.
...
OUTPUT_DIR = Path("/Users/elize/Desktop/量化/outputs/spyder_lightgbm_rolling_pipeline")
...
def safe_to_parquet(df, path):
    ...
    for column in save_df.columns:
        if isinstance(save_df[column].dtype, pd.CategoricalDtype):
            save_df[column] = save_df[column].astype("object")
...
def plot_nav(daily_df):
    ...
    plt.title("LightGBM Rolling-Window Backtest NAV")
    plt.xlabel("Date")
...
def main():
    ...
    safe_to_parquet(raw_features, OUTPUT_DIR / "spyder_raw_features.parquet")

    print("3. Preprocessing cross-section features...")
    features = preprocess_features(raw_features)
    safe_to_parquet(features, OUTPUT_DIR / "spyder_preprocessed_features.parquet")

    print("4. Building forward returns...")
    forward_returns = load_forward_returns(stock_df, index_df)

    print("5. Rolling-window LightGBM training and prediction...")
    predictions = rolling_window_predict(features)
    safe_to_parquet(predictions, OUTPUT_DIR / "spyder_lightgbm_predictions.parquet")
